...
// we need a C++ concept
namespace details {
template <typename T> struct is_callback_fn : std::false_type {};
...
namespace gui {
template <typename T> using setter_fn_type = std::function<void(const T &)>;
...
public:
  ...
  template <typename T> state_pair_type<T> use_state(const T &initial_value) {
  ...
  void use_effect(effect_fn_type effect_fn, std::tuple<Deps...> deps) {
    ...
    if (!m_state.contains<std::tuple<Deps...>>(deps_key)) {
      ...
      if (cleanup_fn) {
        // if the effect returned a cleanup function, we add it to the state
